...
export interface WaveOptions {
  /** Called exactly once to persist the choice (cookie + server). */
...
/**
 * Wave-speed → duration map. Must stay in sync with the server's
 ...
 * stamped by the root layout from the resolved preference (per-user
 * override, else sitewide `appearance_config` default, else `normal`).
 * Falls back to `DEFAULT_DURATION` when the attribute is absent or
 * unrecognised (e.g. server render hasn't stamped it yet, or jsdom in
 * tests) — matches the pre-existing behaviour before wave speed shipped.
 */
